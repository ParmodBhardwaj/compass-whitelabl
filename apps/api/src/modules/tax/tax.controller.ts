...
export class TaxController {
  ...
  /** Sectioned landing payload — sections + top-N docs/links per + FAQs. */
  @Get('dashboard')
  dashboard() {
  ...
  updateDocument(
    ...
    @Body() body: Partial<{ title: string; documentName: string; documentType: string; sectionId: number; status: string }>,
  ) {
  ...
  updateHyperlink(
    @Param('id', ParseIntPipe) id: number,
    @Body() body: Partial<{ title: string; link: string; linkType: string; sectionId: number; status: string }>,
  ) {
